...
def Articulos(request):
    ...
    return render(request,"inicio/arti.html",{'articles': articles})

gstreamer1.0-vaapi libfwupdplugin1
  libgstreamer-plugins-bad1.0-0 libva-wayland2
0 actualizados, 0 nuevos se instalarán, 5 para eliminar y 27 no actualizados.
...
class NuevoArticulo(LoginRequiredMixin,CreateView):
    ...
    template_name="articulos/nuevos_articulo.html"
    success_url=reverse_lazy('articulos')
...
def EditarArticulo(request,pk):
    ...
    if request.method == 'GET':
        if pk in li:
            ...
            return render(request,"articulos/editar_articulos.html",{'article': article})
        else:
            return render(request,"articulos/error.html")
...
def Comentar(request):
    ...
    return redirect("detalle", title=tit, pk=art)
...
def ArticleLike(request,value,pk):
    ...
    return redirect("detalle",title=article.title, pk=pk)
